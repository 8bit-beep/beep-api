...
interface TeacherAttendanceDocs {
    ...
    fun getAttendances(
        @Parameter(description = "실 ID (입력 시 해당 실 스케줄 학생만 조회)") @Positive(message = "실 ID는 양수여야 합니다") roomId: Long?,
        @Parameter(description = "출석 상태 ID(type ID) 필터") @Positive(message = "상태 ID는 양수여야 합니다") statusId: Long?,
