...
- **수기 영역**은 교사가 쓴 내용이며 행사 변경에 영향받지 않는다. 조회 시 위쪽에 온다.
- **자동 영역**은 [교내 행사](#13-교내-행사-event) 등록·수정·삭제 때마다 서버가 통째로 다시 만든다. 여기를 직접 고쳐 보내도 다음 행사 변경 때 덮어써진다. 매일 21:50(Asia/Seoul)에 서버가 비운다. 수기 영역은 이 스케줄의 대상이 아니다.
- `PATCH`로 보낸 전체 텍스트에서 서버가 **뒤쪽** 자동 영역을 떼어내고 나머지를 수기 영역으로 저장한다. **조회로 받은 `content`를 그대로 편집해서 보내면 된다.**
